...
with open(sys.argv[1])as fp:
	for line in fp.readlines():
		if line[0] != '>':
			#seq = line.rstrip()
			#for aa in seq: #also does the same this as the 'for' line below
			for aa in line[:-1]: #need to remove the last line character on each of the new aa therefore [:-1] removes the last line
				if aa != "*" : total += 1
